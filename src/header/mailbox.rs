        ...
        impl Header for $type_name {
            ...
            fn parse_header(raw: &Raw) -> HyperResult<$type_name> {
                raw.one().ok_or(HyperError::Header)
                    ...
                    .and_then(|l| if l.is_empty() { Err(HyperError::Header) } else { Ok(l[0].clone()) })
                    .map($type_name)
            }
    ...
    ( $type_name: ident, $header_name: expr ) => {
        ...
        impl Header for $type_name {
            ...
            fn parse_header(raw: &Raw) -> HyperResult<$type_name> {
                raw.one().ok_or(HyperError::Header)
                    .and_then(parse_mailboxes)
                    .and_then(|l| if l.is_empty() { Err(HyperError::Header) } else { Ok(l) })
                    .map($type_name)
            }
...
fn parse_mailboxes(raw: &[u8]) -> HyperResult<Vec<Mailbox>> {
    if let Ok(src) = from_utf8(raw) {
        if let Ok(mbs) = src.split(',')
            .map(|m| m.trim().parse().map_err(|_| ())
